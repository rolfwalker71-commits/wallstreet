...
import { fabClass, listTileClass, type Chrome } from "@/lib/platform";
...
export function SignalsPage() {
  ...
  const [llm, setLlm] = useState<{ on: boolean; model?: string | null }>({ on: false });
  ...
  useEffect(() => {
    api
      .health()
      .then((h) => setLlm({ on: Boolean(h.llm_enabled), model: h.llm_model }))
      .catch(() => undefined);
  }, []);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground">
            {llm.on
              ? `Aktuelle Empfehlung je Titel (${llm.model}): Watchlist plus Ideen aus den News, jeweils mit Begründung.`
        ...
        <button
          ...
          onClick={run}
          disabled={busy}
          className={`${fabClass(chrome)} inline-flex shrink-0 items-center justify-center gap-2 px-4 font-medium disabled:opacity-60`}
        >
          <Play className="size-5" aria-hidden />
          <span className="sr-only sm:not-sr-only">{busy ? "Läuft…" : "Agenten starten"}</span>
        </button>
      </div>
