...
import { CLASS_LABEL, number, pct, recAccentClass, when } from "@/lib/format";
...
export function EmpfehlungenPage() {
  ...
  const load = async (refresh = false) => {
    const data = refresh ? await api.refreshPicks() : await api.picks();
    setItems(data);
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        >
          ...
          <span>{busy ? "Bewertet…" : "Neu bewerten"}</span>
        </button>
      </div>
      ...
      {loading ? (
        <p className={`${listTileClass(chrome)} px-5 py-8 text-muted-foreground`}>
          Stellt die Kaufideen zusammen…
        </p>
      ) : null}

      {!loading && items.length === 0 && !error ? (
        <p className={`${listTileClass(chrome)} px-5 py-8 text-muted-foreground`}>
          Noch keine belastbare Kaufidee. Starte die Agenten unter Signale oder tippe «Neu bewerten».
        </p>
      ) : null}
        ...
        {items.map((rec, index) => (
          ...
          >
            <Link to={`/signals/${rec.id}`} className="block">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-gain">Kauf {index + 1}</p>
